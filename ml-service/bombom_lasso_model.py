...
class BombomLassoModel:
    ...
    def load_and_prepare_data(self):
        ...
        for window in [3, 6, 12]:
            bombom_data[f'quantity_ma{window}'] = bombom_data['quantity'].rolling(window=window, min_periods=1).mean()
            bombom_data[f'quantity_std{window}'] = bombom_data['quantity'].rolling(window=window, min_periods=1).std()
        
        # 4. Trend and growth features
        bombom_data['trend'] = np.arange(len(bombom_data))
        bombom_data['trend_squared'] = bombom_data['trend'] ** 2
        bombom_data['quantity_pct_change'] = bombom_data['quantity'].pct_change()
        bombom_data['quantity_diff'] = bombom_data['quantity'].diff()
        ...
        if len(bombom_data) >= 24:
            ...
            bombom_data['yoy_growth'] = (bombom_data['quantity'] - bombom_data['quantity_yoy']) / bombom_data['quantity_yoy'].replace(0, np.nan)
