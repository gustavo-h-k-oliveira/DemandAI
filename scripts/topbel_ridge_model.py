...
class TopbelRidgeModel:
    ...
    def load_and_prepare_data(self):
        ...
        # 3. Smooth rolling statistics (longer windows for stability) - past-only
        past_q = topbel_data['quantity'].shift(1)
        for window in [3, 6, 12]:
            topbel_data[f'quantity_ma{window}'] = past_q.rolling(window=window, min_periods=max(1, window//2)).mean()
            topbel_data[f'quantity_ewm{window}'] = past_q.ewm(span=window).mean()
        ...
        topbel_data['quantity_pct_change'] = past_q.pct_change().fillna(0)
        ...
        # 8. Stability features (past-only)
        topbel_data['quantity_stability_3'] = past_q.rolling(window=3).std().fillna(0)
        topbel_data['quantity_stability_6'] = past_q.rolling(window=6).std().fillna(0)
        ...
        if len(topbel_data) >= 24:
            ...
            topbel_data['yoy_growth'] = (((prev_q - prev_yoy) / prev_yoy.replace(0, np.nan))).fillna(0)
    ...
    def find_optimal_conservative_alpha(self, X, y):
        ...
        print("Training RidgeCV (with scaler in pipeline) for optimal conservative alpha selection...")
        ridge_cv.fit(X, y)

        optimal_alpha = ridge_cv.named_steps['model'].alpha_
        cv_score = ridge_cv.named_steps['model'].score(ridge_cv.named_steps['scaler'].transform(X), y)
    ...
    def save_model(self, filename='topbel_ridge_conservative_model.pkl'):
        ...
        print(f"\n✅ Ridge Conservative model saved to: {out_path}")
